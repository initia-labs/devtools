...
class EVMSendNativeOperation implements INewOperation {
    vm = 'evm'
    operation = 'send-native'
    description = 'Sends an Native OFT message'
    ...
    async impl(args: any): Promise<void> {
        await SendNativeOFT(args)
    }
}

const NewOperation = new EVMSendNativeOperation()
export { NewOperation }

async function SendNativeOFT(args: any): Promise<MessagingFee> {
    ...
    const options = Options.newOptions().addExecutorLzReceiveOption(500000) // .addExecutorComposeOption(0, 800_000);
